...
const userDetails = ref({}) // Firestore user details
  ...
  <aside class="hidden xl:flex xl:flex-col xl:w-72 border-l bg-white px-4 py-6 space-y-8">
    ...
    <div class="bg-gray-50 p-4 rounded-lg shadow-sm">
      <div class="flex items-center space-x-3">
        <img
          :src="userDetails.photoURL || 'https://images.unsplash.com/photo-1499714608240-22fc6ad53fb2?auto=format&fit=facearea&facepad=3&w=64&h=64&q=80'"
          alt="User avatar"
          class="w-12 h-12 rounded-full"
        />
        <div>
          <h4 class="font-semibold text-gray-800">
            {{ userDetails.firstName && userDetails.lastName ? userDetails.firstName + ' ' + userDetails.lastName : 'Guest User' }}
          </h4>
          <p class="text-sm text-gray-500">
            {{ userDetails.course ? userDetails.course + ', Class of ' + userDetails.year : 'Not enrolled' }}
          </p>
        </div>
      </div>
      <div class="mt-4 text-gray-600 text-sm leading-snug">
        Welcome back! Here’s your quick overview: 3 new messages, 2 upcoming deadlines, and 1 event today.
      </div>
          ...
          <form class="flex items-center">
            ...
            <button type="submit" class="ml-2 text-indigo-600 hover:text-indigo-800 focus:outline-none">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M2 12l1.82 1.275c.213.149.213.5 0 .65L2 15.2m20-6v6m0 0l-1.82-1.275c-.213-.149-.213-.5 0-.65L22 12"/>
